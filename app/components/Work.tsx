                ...
                {workData.map((project, index) => (
                    ...
                    >
                        <div className='bg-white w-10/12 rounded-md absolute bottom-5 left-1/2 -translate-x-1/2 py-3 px-3 flex items-center justify-between duration-500 group-hover:bottom-7'>
                            <div>
                                <h2 className='font-semibold'>{project.title}
                                    <p className='text-sm text-gray-700'>{project.description}</p>
                                </h2>
                            </div>
                            <div className='flex items-centerjustify-center border rounded-full border-black w-9  aspect-square shadow-[2px_2px_0_#000] group-hover:bg-lime-300 duration-500'>
                                <Image
                                    src={send_icon}
                                    alt='send icon'
                                    className='w-5' />
